...
var usage = fs.readFileSync(path.join(__dirname, 'docs/usage.txt'), 'utf8')
...
cli.parse = function parse (tokens, options) {
  tokens = [].concat(tokens) // copy this and leave argv intact
  var command = {}
  var token
  var symbol
...
cli.run = function () {
  var argv = yargs.argv
  var env = argv.e || argv.environment
  var tokens = [].concat(argv._)
  var command = cli.parse(tokens)
