...
function tag (done) {
  rclient.set('version', server.settings.version, function (err) {
    done(err);
  });
}
  ...
  multi.exec(function (err, results) {
    ...
    async.parallel(ops, function (err, results) {
      if (err) {
        ...
        console.log('Already at v%s.', server.settings.version);
      }

      else {
        console.log('Anvil Connect migrated to v%s.', server.settings.version);
      }
