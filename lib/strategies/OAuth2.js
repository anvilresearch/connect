...
function authenticate (req, options) {
  ...
  if (error && error === 'access_denied') {
    return strategy.fail({ message: req.query.error_description });
  }
...
// this is temporary; assumes a well formed
// x-form-urlencoded input
...
function userInfo (token, done) {
  ...
  // Let's refactor this to construct the header
  // from an auth method object. e.g.,
  // endpoints: {
  //   user: {
  //     url: '...',
  //     method: 'GET',
  ...
  // }
  if (auth === 'oauth_token') {
    req.set('Authorization', 'OAuth ' + token);
  }

  // Authenticate with query string
  if (auth === 'access_token') {
    var credentials = (provider.accessTokenName || 'access_token') + '=' + token;
