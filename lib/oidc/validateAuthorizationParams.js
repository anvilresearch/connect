...
function validateAuthorizationParams (req, res, next) {
  ...
  // invalid nonce
  if (requiresNonce(params.response_type) &&
      !validNonce(params.nonce, req.cookies['connect.sid'])) {

    // log the valid nonce to console for dev convenience
    var shasum = crypto.createHash('sha1');
    shasum.update(req.cookies['connect.sid']);
    console.log('Valid nonce:', shasum.digest('hex'));
...
function validNonce (nonce, cookie) {
  var shasum = crypto.createHash('sha1');
  shasum.update(cookie);
  return (nonce === shasum.digest('hex'));
}
